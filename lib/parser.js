    ...
    return q.Promise(function (resolve, reject) {
        toArray(destinationStream).then(function (arr) {
            ...
            parser.parseString(content, function (err, res) {
                ...
                let destination = _.get(res, 'destinations.destination', undefined);
                if (destination) {
                    resolve(xmltojson.destinationToJson(destination));
                } else {
                    reject(new Error('Destinations should not be null'));
                }
