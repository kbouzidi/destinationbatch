 ...
 * @private
 */
function _parseDestinationHistory(historyData, historyTag, res) {
    if (!res) {
        res = [];
    }
    if (historyData) {
        if (historyData.length > 0) {
            for (var i = 0; i < historyData.length; i++) {
                _parseDestinationHistory(historyData[i], historyTag, res);
            }
        } else if (historyData[historyTag]) {
            res.push(historyData[historyTag])
        }
    }
    return res;
}
...
exports.destinationToJson = function (destinationsList) {
    ...
    for (var i = 0; i < destinationsList.length; i++) {
        ...
        if (destination.history) {
            var result = _parseDestinationHistory(destination.history, 'history');
